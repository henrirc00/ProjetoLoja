...
if(!empty($data->foto) && !empty($data->id)){
    ...
    if($usuario->alterarfoto()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"Foto do usuario atualizada com sucesso!"));
    }
    else{
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não foi possivel atualizara foto."));
    }
}
else{
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Voce precisa passar todos os dados para alterarfoto"));
}
