...
if(!empty($data->id)){
    ...
    if($detalhepedido->Apagar()){
        ...
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não foi possivel apagar detalhepedido."));
    }
}
else{
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Voce precisa passar todos os dados para apagar o detalhepedido"));
}
